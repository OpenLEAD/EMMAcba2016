...
prismatic joints (P). The first, or primary rail, is parallel to the turbine
axis, and it is responsible for the transportation of the manipulator from
...
The hatch and turbine' space limits the size of the rail in terms of weight
and geometry, thus a modular concept was adopted, such that the small modular
parts can be easily and manually assembled inside the turbine. The rails are
formed by modules, such that each module contains all components
necessary to support, transport and position the manipulator along its
direction. Thus, it is possible to simply assemble modules in sequence to
increase the prismatic joint overall length. 

It was adopted a two parallel profiled rail system with a four carriage setup.
%\cite{SKF_2013}.
This configuration provides force couples wich eliminates the reaction
moments in each carriage. The frame structure is formed by commercial
structural aluminum profile. The main advantages regarding this choice are:
corrosion resistance, lightweight, geometric flexibility, and modularity, which
make possible to modify the rail configuration by changing few parts or adding
anchor arms.

The resulting structure is a slender and lightweight frame, thus a
careful analysis is necessary to evaluate the structural integrity and rigidity in
