...
regular full blade cover, gun's output diameter flame \cite{li2002effect}.
...
position. However, \textit{in situ} HVOF coating represents
...
axis bulb type turbines, and its points of interest for solution development
are: 1) the variable pitch propeller, or Kaplan \textbf{blades} (2500~mm tall
and 3000~mm wide); 2) the variable pitch guide vanes, or \textbf{wicket gates};
3) the \textbf{runner area}; 4) the \textbf{draft tube}; and 5) the 800~mm access for regular maintenance, or \textbf{hatch}. A 3D CAD model of the turbine was built with
...
proposed robotic system is to hard coat the two sides of one blade. EMMA must
comply with the HVOF requirements, it must overcome the constraints and logistical challenges of the environment, and operate in
autonomously mode. 
